...
import com.example.dailyTarget.enums.CellStyleEnum;
...
public class ExcelUtil {
    ...
    public static void exportExcel (String sheetName, Class<?> pojoClass, List<?> dateCollection,
                                    OutputStream outputStream, String[] excludeColumn, Workbook workbook,
                                    String[] specialMergeValue) throws Exception{
        try {
            //使用userModel模式实现的，当excel文档出现10万级别的大数据文件时可能导致oom内存溢出
            exportExcelInUserModel(sheetName, pojoClass, dateCollection, outputStream, excludeColumn, workbook, specialMergeValue);
            //使用eventModel模式实现，可以一边读一边处理，效率较高，但实现复杂
        } catch (Exception e) {
            ExcelUtil.exceptionExportExcel(sheetName,outputStream,e.getMessage());
            throw new RuntimeException("导出失败！", e);
        }

    }

    public static void exceptionExportExcel(String sheetName,
                                              OutputStream outputStream, String exceptionMessage) throws Exception{
        //变量初始化
        //声明一个工作簿
        ...
        //设置样式
        CellStyle[] cellStyleArray = createCellStyleArray(workbook,null);
        Row row = sheet.createRow(0);
        Cell cell = row.createCell(0);

        Font titleFont = workbook.createFont();
        titleFont.setBold(true);
        RichTextString text = new XSSFRichTextString("错误信息");
        text.applyFont(titleFont);
        cell.setCellValue(text);
        cell.setCellStyle(cellStyleArray[0]);
//        setCellValueByObject(cell, "错误信息", cellStyleArray, null);

        Row row1 = sheet.createRow(1);
        Cell cell1 = row1.createCell(0);
        setCellValueByObject(cell1, exceptionMessage, cellStyleArray, null);
     ...
     *      specialMergeValue 特殊合并值，当此值出现，循环查找同行下一个元素值，直到不是此值为止，所有和此值一样的值做合并操作，用于小计/合计横行合并
    ...
    public static void exportExcelInUserModel(String sheetName, Class<?> pojoClass, List<?> dateList,
                                              OutputStream outputStream, String[] excludeColumn, Workbook wk,
                                              String[] specialMergeValue) throws Exception{
        //数据非空验证
        ...
        //自定义数据格式map，key为唯一字段名列表uniqueFieldName，value为format
        Map<String,String> selfFormatMap = new HashMap<>();

        //得到目标类的所有字段列表
//        Field[] fields = pojoClass.getDeclaredFields();
        //获取当前实体类及其父类其超类。。。直到取不到上级父类所有的字段
        Field[] fields = getAllFields(pojoClass);
        Map<String, Method> classAllMethodsMap = getAllMethods(pojoClass);
...
//                Method getMethod = pojoClass.getDeclaredMethod(getMethodName.toString());
                Method getMethod = classAllMethodsMap.get(getMethodName.toString());
        ...
        //迭代遍历集合，写入数据。如果集合为空不会进行此循环直接输出，将标题行输出到excel中
...
//                Field declaredField = pojoClass.getDeclaredField(uniqueFieldName.get(j));
//                declaredField.setAccessible(true);
//                Object value = declaredField.get(dataObject);
...
//                        (i != dateList.size()-1 && Objects.equals(value, declaredField.get(dateList.get(i+1))))
...
//                        if (k < dateList.size()-1 && Objects.equals(value, declaredField.get(dateList.get(k+1)))) {
                            currentMergeNum++;
                        } else {
                            break;
                        }
...
//                        }
            ...
            //specialMergeValue 特殊合并值
            if(specialMergeValue!=null && specialMergeValue.length>0 && uniqueFieldName.size()>0){
                //specialMergeValue 特殊合并值处理 参数转换成set方便后续处理
                HashSet<String> specialMergeValueSet = new HashSet<>(Arrays.asList(specialMergeValue));
                ExcelUtil.specialMergeValueDispose(uniqueFieldName,methodObject,dataObject,specialMergeValueSet,i,sheet);
            }
        }
        workbook.write(outputStream);
        //todo 测试
...
//        workbook.write(out2);
    ...
    /**
     * specialMergeValue 特殊合并值
     * @param uniqueFieldName 唯一字段名列表：主要用于字段上的特殊限制，如合并标识，后续存入此唯一值在set/map数据插入时开启判断
     * @param methodObject  用于拿到dataCollection中字段值，对应导出字段在实体类中的get方法
     * @param dataObject 当前行数据对象实体 sheet.createRow(index)
     * @param specialMergeValueSet  specialMergeValue 特殊合并值处理,参数转换成set方便后续处理
     * @param index 遍历数据list集合的指针，从0开始
     * @param sheet 声明的工作表格
     */
    private static void specialMergeValueDispose(List<String> uniqueFieldName,List<Method> methodObject,Object dataObject,
                                          HashSet<String> specialMergeValueSet,int index,Sheet sheet)throws Exception {
        //第一个空格的值
        Object value = methodObject.get(0).invoke(dataObject);//小结合计肯定是在第一格开始合并的
        if(value!=null && specialMergeValueSet.contains(String.valueOf(value))){//值不为空，且就是特殊合并值数值的值
            //开始特殊值合并操作
//            String specialValue = String.valueOf(value);
            int mergeNum = 0;//需要合并的数量，指针从0开始
            for(int j = 1; j < uniqueFieldName.size(); j++){//1开始判断，从第二格子开始判断
                if(Objects.equals(methodObject.get(j).invoke(dataObject),value)){
                    mergeNum++;
                }else{
                    break;
                }
            ...
            //进行合并操作
            CellRangeAddress region = new CellRangeAddress(index+1,index+1,0, mergeNum);
            sheet.addMergedRegion(region);
        }
    }
    ...
    public static void setCellValueByObject(Cell cell, Object value, CellStyle[] cellStyleArray,
        ...
        if (value instanceof String){//字符串
            ...
            throw new JeecgBootException("此单元格数据类型无法解析，请联系管理员: " + value.toString());
        }
    }
...
//        Class clazz = object.getClass();
        ...
        Map<String, Method> collect = methodList.stream().collect(Collectors.toMap(i -> i.getName(), Function.identity(),(v1,v2)->v1));
        return collect;
    }
